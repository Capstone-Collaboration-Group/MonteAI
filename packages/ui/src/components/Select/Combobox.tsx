...
export function Combobox({ options, value: ctrlVal, onChange, placeholder = "Search...", label, error, disabled = false, className = "" }: ComboboxProps) {
      ...
      <div className="relative">
        <div
          ...
          className={`w-full px-3.5 py-2.5 rounded-lg border text-sm flex items-center gap-2 bg-surface text-on-surface transition-colors focus-within:ring-2 ${
            error ? "border-[#FF0000] focus-within:ring-[#FF0000]/20" : "border-outline/30 focus-within:border-[#0D7856] focus-within:ring-[#0D7856]/20 dark:focus-within:border-[#5CC29A]"
          } ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
        >
          <svg className="w-4 h-4 text-on-surface-variant shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
          {isOpen ? (
            <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search..." className="w-full bg-transparent outline-none text-on-surface text-sm" />
          ) : (
            <span className={`w-full truncate ${selectedOption ? "text-on-surface" : "text-on-surface-variant/60"}`}>{selectedOption ? selectedOption.label : placeholder}</span>
          )}
          <svg className={`w-4 h-4 text-on-surface-variant transition-transform shrink-0 ${isOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
        </div>
        ...
        {isOpen && (
          <div className="absolute z-50 w-full mt-1 max-h-60 overflow-y-auto py-1 bg-surface dark:bg-surface-container-low text-on-surface rounded-xl shadow-xl border border-outline/20 animate-in fade-in duration-100">
            {filtered.length === 0 ? (
              <div className="px-4 py-3 text-xs text-center text-on-surface-variant">No results found</div>
            ) : (
              filtered.map((opt) => (
                <button
                  key={opt.value}
