...
export function SubmissionHealthCard( {stats}: SubmissionHealthCardProps) { 
    ...
    return (
      ...
      <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#9DB5A9]">
        Submission Health
      </p>
 
      <p className="mt-3 font-serif text-4xl font-semibold leading-none">
        {pct}%
        <span className="ml-2 align-middle text-base font-sans font-normal text-[#9DB5A9]">
          Approval
        </span>
      </p>
 
      <p className="mt-3 text-sm leading-relaxed text-[#C7D6CC]">
        {stats.note ??
      ...
      <div className="mt-auto pt-8">
        <div className="relative h-px w-full bg-[#3A5A4C]">
          <div
            className="absolute inset-y-0 left-0 h-px bg-[#D9B871]"
            style={{ width: `${pct}%` }}
          />
          <div
            className="absolute -top-1 h-2.5 w-2.5 -translate-x-1/2 rounded-full bg-[#D9B871]"
            style={{ left: `${pct}%` }}
          />
        </div>
        <div className="mt-2 flex justify-between">
          {TICKS.map((t) => (
            <span key={t} className="text-[10px] font-medium text-[#7F9A8C]">
              {t}
            </span>
          ))}
        </div>
