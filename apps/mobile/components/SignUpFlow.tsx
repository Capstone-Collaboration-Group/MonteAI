  ...
  return (
    <View style={s.dotsRow}>
      {Array.from({ length: TOTAL_FORM_STEPS }, (_, i) => (
        <View
          key={i}
          style={[s.dot, { backgroundColor: i < step ? primary : "#d1d5db" }]}
        />
      ))}
    </View>
  );
...
export default function SignUpFlow({
  ...
  const validateStep = (): boolean => {
    ...
    if (step === 2) {
      ...
      else if (!/^[A-Za-z]$/.test(section.trim()))
        next.section = "Use a single letter (e.g. A).";
    } else if (step === 4) {
      ...
      if (!value) next.email = "Email is required.";
        ...
        next.email = `Only accept ${ALLOWED_EMAIL_DOMAIN} emails.`;
    }
    setErrors(next);
  ...
  const buildPayload = (): SignUpPayload => {
    ...
    const fullName =
      `${firstName.trim()}${mi ? ` ${mi}.` : ""} ${lastName.trim()}${cleanSuffix ? ` ${cleanSuffix}` : ""}`
        .replace(/\s+/g, " ")
        .trim();
    return {
      fullName,
          ...
          >
            ...
            {step === 1 ? (
              <View style={s.welcome}>
                ...
                <Text style={[s.subhead, { color: body }]}>
                  Online Thesis Library System
                </Text>
                <View style={s.heroMargin}>
                  <Text style={[s.hero, { color: heading }]}>
                    Create your{"\n"}account
                  </Text>
                  <Text style={[s.desc, { color: body }]}>
                    Join MonteSkolar and{"\n"}access a world of research,{"\n"}
              ...
              <View style={s.stepBody}>
                ...
                <Text style={[s.stepOf, { color: "#6b7280" }]}>
                ...
                {step === 2 && (
                  <>
                    ...
                    <TextField
                      ...
                      value={suffix}
                      onChangeText={(v) =>
                        setSuffix(v.replace(/\s/g, "").slice(0, 4))
                      }
                ...
                {step === 3 && (
                  <>
                    ...
                    <SelectDropdown
                      ...
                      onSelect={(label) => {
                        const m = YEAR_LEVELS.find((y) => y.label === label);
                        if (m) setYearLevel(m.value);
                      }}
                ...
                {step === 4 && (
                  <>
                    ...
                    >
                      <View
                        style={[
                          s.inputRow,
                          {
                            ...
                            borderColor: errors.password ? "#ba1a1a" : outline,
                          },
                        ]}
                      >
                        <TextInput
                          ...
                          autoCapitalize="none"
                          style={[s.inputFlex, { color: heading }]}
                        />
                        <Pressable
                      ...
                      {!errors.password ? (
                        <Text style={[s.hint, { color: "#6b7280" }]}>
                ...
                {step === 5 && (
                  <>
                    <TextField
                      ...
                      placeholder={`Enter your ${ALLOWED_EMAIL_DOMAIN}`}
                    ...
                    {!errors.email ? (
                      <Text style={[s.hint, { color: "#6b7280" }]}>
                        Only accept {ALLOWED_EMAIL_DOMAIN} emails
                      </Text>
                    ) : null}
                  </>
                )}
            ...
            <View style={s.actions}>
              <PrimaryButton
                label={step === TOTAL_FORM_STEPS ? "Create Account" : "Next"}
                onPress={handleNext}
                accessibilityLabel={
                  step === TOTAL_FORM_STEPS ? "Create Account" : "Next"
                }
              />
              {step === TOTAL_FORM_STEPS ? (
                <Text style={[s.loginRow, { color: body }]}>
                  ...
                  <Text
                    ...
                    style={[s.loginLink, { color: primary }]}
                  >
                    Login here
                  </Text>
                </Text>
